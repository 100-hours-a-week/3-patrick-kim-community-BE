#!/usr/bin/env python3
"""PetStar Portfolio DOCX Generator v4 — v1 design + muted diagrams"""
...
# ── Diagram-only palette (muted, technical) ──
D_DARK    = '#2D3748'   # darkest bar / fill
D_MID     = '#4A5568'   # mid-tone
D_SOFT    = '#718096'   # softer
D_MUTED   = '#A0AEC0'   # muted / non-selected
D_PALE    = '#CBD5E0'   # pale accent
D_BG      = '#EDF2F7'   # diagram background region
D_LINE    = '#A0AEC0'   # arrows / lines
D_CHOSEN  = '#2B6CB0'   # single muted blue for "chosen" items
D_TEXT    = '#1A202C'   # text on chart
...
def create_optimization_waterfall():
    ...
    colors = [D_DARK, D_MID, D_SOFT, D_CHOSEN]
    reductions = ['', '81%↓', '73%↓', '38%↓']
    ...
    for i, (bar, val, red) in enumerate(zip(bars, values, reductions)):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 80,
                ...
                fontweight='bold', color=D_TEXT, fontfamily=FONT_KR)
        if red:
    ...
    ax.annotate('', xy=(3, 300), xytext=(0, 4500),
                arrowprops=dict(arrowstyle='->', color=D_CHOSEN, lw=2.5,
                                connectionstyle='arc3,rad=-0.3'))
    ax.text(2.1, 3200, '97% 감소', fontsize=14, fontweight='bold',
            color=D_CHOSEN, ha='center', fontfamily=FONT_KR, rotation=-30)
...
def create_query_comparison():
    ...
    cats2 = ['Before\n(50 VUs)', 'After\n(300 VUs)']
    vals2 = [10.9, 308]
    bars2 = ax2.bar(cats2, vals2, color=[D_MUTED, D_CHOSEN], width=0.5,
...
def create_concurrency_comparison():
    ...
    # Success rate
    rates = [100, 16, 94]
    colors_rate = [D_CHOSEN, D_MUTED, D_MUTED]
    ...
    # Response time
    times = [1730, 885, 2980]
    colors_time = [D_CHOSEN, D_MUTED, D_MUTED]
...
def create_retry_storm_diagram():
    ...
    ax1.set_title('낙관적 락: Retry Storm', fontsize=13, fontweight='bold',
                  color=D_DARK, fontfamily=FONT_KR, pad=10)
    ...
    for i, (y, label) in enumerate(zip(users_y, user_labels)):
        ax1.add_patch(FancyBboxPatch((0.3, y-0.25), 1.4, 0.5, boxstyle="round,pad=0.05",
                                    facecolor=D_BG, edgecolor=D_MID, linewidth=1))
        ax1.text(1.0, y, label, ha='center', va='center', fontsize=8,
                 color=D_DARK, fontweight='bold', fontfamily=FONT_KR)

    for y in users_y[:3]:
        ax1.annotate('', xy=(4.5, y), xytext=(1.8, y),
                     arrowprops=dict(arrowstyle='->', color=D_MID, lw=1.2))
        ax1.text(3.2, y+0.2, '충돌', fontsize=7, color=D_DARK,
                 ha='center', fontfamily=FONT_KR)
        ax1.annotate('', xy=(7.0, y-0.3), xytext=(4.8, y-0.3),
                     arrowprops=dict(arrowstyle='->', color=D_SOFT, lw=1.2, linestyle='--'))
        ax1.text(5.9, y-0.55, '재시도', fontsize=7, color=D_SOFT,
                 ha='center', fontfamily=FONT_KR)
        ax1.annotate('', xy=(9.5, y-0.6), xytext=(7.3, y-0.6),
                     arrowprops=dict(arrowstyle='->', color=D_MUTED, lw=1.2, linestyle='--'))

    ax1.text(5, 1.8, '충돌 → 재시도 → 재충돌', fontsize=10, fontweight='bold',
             color=D_DARK, ha='center', fontfamily=FONT_KR)
    ax1.text(5, 1.0, '지수적 경쟁 증가', fontsize=9,
             color=D_MID, ha='center', fontfamily=FONT_KR)
...
def create_async_architecture():
    ...
    ax.add_patch(FancyBboxPatch((0.2, 3.5), 11.6, 2.7, boxstyle="round,pad=0.15",
                                facecolor=D_BG, edgecolor=D_PALE, linewidth=1.5,
                                linestyle='--'))
    ax.text(6, 6.0, '빠른 경로 (Fast Path) — 즉시 응답', fontsize=10,
            color=D_DARK, ha='center', fontweight='bold', fontfamily=FONT_KR)

    draw_box(ax, 0.5, 4.0, 1.6, 1.2, '클라이언트', '투표 요청', D_DARK)
    draw_box(ax, 3.0, 4.0, 2.0, 1.2, 'Spring Boot', 'API Server', D_MID)
    draw_box(ax, 6.2, 4.0, 2.0, 1.2, 'Redis', 'Sorted Set', D_SOFT)

    draw_arrow(ax, 2.1, 4.6, 3.0, 4.6, '투표', D_DARK)
    draw_arrow(ax, 5.0, 4.6, 6.2, 4.6, 'ZINCRBY', D_MID)
    draw_arrow(ax, 6.2, 4.3, 5.0, 4.3, '즉시 반영', D_MID)
    draw_arrow(ax, 3.0, 4.3, 2.1, 4.3, '응답', D_DARK)

    # Response time badge
    ax.add_patch(FancyBboxPatch((9.0, 4.2), 2.5, 0.8, boxstyle="round,pad=0.1",
                                facecolor=D_CHOSEN, edgecolor=D_CHOSEN))
    ax.text(10.25, 4.6, '~5ms 응답', fontsize=12, fontweight='bold',
            color=C_WHITE, ha='center', va='center', fontfamily=FONT_KR)

    # ── Slow Path (bottom) ──
    ax.add_patch(FancyBboxPatch((0.2, 0.3), 11.6, 2.7, boxstyle="round,pad=0.15",
                                facecolor='#F7FAFC', edgecolor=D_PALE, linewidth=1.5,
                                linestyle='--'))
    ax.text(6, 2.8, '느린 경로 (Slow Path) — 비동기 영속화', fontsize=10,
            color=D_SOFT, ha='center', fontweight='bold', fontfamily=FONT_KR)

    draw_box(ax, 0.5, 0.7, 2.0, 1.2, 'SQS Queue', '메시지 발행', D_SOFT)
    draw_box(ax, 3.5, 0.7, 2.0, 1.2, 'Consumer', '비동기 처리', D_MID)
    draw_box(ax, 6.5, 0.7, 2.0, 1.2, 'MySQL RDS', 'Source of Truth', D_DARK)

    draw_arrow(ax, 4.0, 4.0, 1.5, 1.9, 'Fire & Forget', D_SOFT)
    draw_arrow(ax, 2.5, 1.3, 3.5, 1.3, '폴링', D_MID)
    draw_arrow(ax, 5.5, 1.3, 6.5, 1.3, 'DB 저장', D_DARK)

    # DLQ
    draw_box(ax, 3.5, -0.1, 2.0, 0.5, 'DLQ (3회 재시도 → 14일 보관)', '', D_MUTED, C_WHITE)

    # Scheduler
    draw_box(ax, 9.2, 1.0, 2.3, 0.8, 'Scheduler', '5분 정합성 검증', D_MUTED)
    draw_arrow(ax, 9.2, 1.6, 8.5, 1.3, '', D_LINE)
    draw_arrow(ax, 10.0, 1.8, 7.5, 4.0, '검증', D_LINE, lw=1.0)
...
def create_performance_reversal():
    ...
    colors = [D_MUTED, D_MID, D_DARK, D_CHOSEN]
    ...
    for bar, val, col in zip(bars, values, colors):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 30,
                f'{val:,}ms', ha='center', fontsize=12, fontweight='bold',
                color=D_TEXT, fontfamily=FONT_KR)

    # Highlight the reversal
    ax.annotate('성능 역전', xy=(2, 1300), xytext=(2.8, 1600),
                fontsize=13, fontweight='bold', color=D_DARK,
                fontfamily=FONT_KR,
                arrowprops=dict(arrowstyle='->', color=D_DARK, lw=2))

    # Explanation box
    ax.add_patch(FancyBboxPatch((2.55, 200), 1.5, 600, boxstyle="round,pad=0.1",
                                facecolor=D_BG, edgecolor=D_MID, linewidth=1.5,
                                linestyle='--', alpha=0.7))
...
def create_connection_model_diagram():
    ...
    ax1.set_xlim(0, 10); ax1.set_ylim(0, 10)
    ax1.axis('off')
    ax1.set_title('Lettuce — 단일 커넥션 멀티플렉싱', fontsize=13, fontweight='bold',
                  color=D_DARK, fontfamily=FONT_KR, pad=10)
    ...
    for i in range(5):
        ...
        ax1.text(1.2, y+0.05, f'요청 {i+1}', ha='center', va='center',
                 fontsize=8, color=D_DARK, fontweight='bold', fontfamily=FONT_KR)
        ax1.annotate('', xy=(4.5, 5.0), xytext=(2.1, y+0.05),
                     arrowprops=dict(arrowstyle='->', color=D_PALE, lw=1))

    ax1.add_patch(FancyBboxPatch((4.3, 4.0), 2.5, 2.0, boxstyle="round,pad=0.1",
                                facecolor=D_DARK, edgecolor=D_DARK, linewidth=2))
    ...
    ax2.set_xlim(0, 10); ax2.set_ylim(0, 10)
    ax2.axis('off')
    ax2.set_title('HikariCP — 30개 커넥션 풀', fontsize=13, fontweight='bold',
                  color=D_DARK, fontfamily=FONT_KR, pad=10)

    for i in range(5):
        y = 8.5 - i * 1.2
        ax2.add_patch(FancyBboxPatch((0.3, y-0.2), 1.8, 0.5, boxstyle="round,pad=0.05",
                                    facecolor=D_BG, edgecolor=D_CHOSEN, linewidth=1))
        ax2.text(1.2, y+0.05, f'요청 {i+1}', ha='center', va='center',
                 fontsize=8, color=D_CHOSEN, fontweight='bold', fontfamily=FONT_KR)
    ...
    for i, cy in enumerate(conn_y_positions):
        ax2.add_patch(FancyBboxPatch((3.8, cy-0.2), 1.6, 0.5, boxstyle="round,pad=0.05",
                                    facecolor=D_CHOSEN, edgecolor=D_CHOSEN, linewidth=1))
    ...
    ax2.add_patch(FancyBboxPatch((3.5, 2.0), 2.2, 6.5, boxstyle="round,pad=0.15",
                                facecolor='none', edgecolor=D_CHOSEN, linewidth=2,
                                linestyle='--'))
    ax2.text(4.6, 2.3, '30개 풀', fontsize=9, color=D_CHOSEN,
    ...
    ax2.add_patch(FancyBboxPatch((6.2, 3.8), 2.0, 2.5, boxstyle="round,pad=0.08",
                                facecolor=D_CHOSEN, edgecolor=D_CHOSEN, linewidth=1.5))
...
def build_document():
    ...
    # ── Save ──
    output_path = os.path.join(OUT_DIR, 'PetStar_Portfolio_v4.docx')
