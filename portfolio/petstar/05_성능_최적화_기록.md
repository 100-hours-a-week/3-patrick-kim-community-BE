...
- **데이터**: Member 10K, Pet 15K, Challenge 50, Entry 100K, Vote 1M

### 부하별 성능 비교

| VUs | challenge_list | entries | ranking | **전체 p95** | throughput | 에러율 |
|-----|----------------|---------|---------|--------------|------------|--------|
| 50 | 241ms | 38ms | 59ms | **65ms** | 31 req/s | 0% |
| 100 | 87ms | 83ms | 91ms | **88ms** | 62 req/s | 0% |
| 200 | 133ms | 40ms | 53ms | **62ms** | 123 req/s | 0% |

### Before vs After 비교

| API | Before (50 VUs) | After (200 VUs) | 개선율 |
|-----|-----------------|-----------------|--------|
| **ranking** | 9.58s | 53ms | **99.4%** |
| **entries** | 2.84s | 40ms | **98.6%** |
| **throughput** | 10 req/s | 123 req/s | **12배** |

### 동시 접속자 200명 = 어느 정도 규모?

```
동접 200명 ≈ DAU 2,000~4,000명 ≈ MAU 10,000~40,000명
→ 소규모 스타트업 ~ 초기 서비스 수준
```

### 결론
- **애플리케이션 레벨 최적화 완료**: 200 VUs까지 p95 < 100ms 안정적
- **추가 트래픽 대응**: 인프라 스케일링 (스케일업/아웃) 필요